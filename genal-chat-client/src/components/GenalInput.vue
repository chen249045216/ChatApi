...
import { sendImageMessage } from '@/api/apis';
...
export default class GenalInput extends Vue {
  ...
  @chatModule.Getter('signal') signal: signalR.HubConnection;
  ...
  preSendMessage() {
    ...
    if (this.activeRoom.groupId) {
      this.sendMessage({ type: 'group', message: this.text, messageType: 0 });
    } else {
      this.sendMessage({ type: 'friend', message: this.text, messageType: 0 });
    }
    this.text = '';
  }
  ...
  sendMessage(data: SendMessage) {
    if (data.type === 'group') {
      this.signal.invoke('GroupMessage', {
      ...
      this.signal.invoke('friendMessage', {
        userId: this.user.userId,
  ...
  async handleImgUpload(imageFile: File) {
    ...
    let type = this.activeRoom.groupId ? 'group' : 'friend';
    let toId = type === 'group' ? this.activeRoom.groupId : this.activeRoom.userId;
    let image = new Image();
    let url = window.URL || window.webkitURL;
    image.src = url.createObjectURL(imageFile);
    image.onload = () => {
      let imageSize: ImageSize = this.getImageSize({ width: image.width, height: image.height });
      formData.append('fileInput', imageFile, imageFile.name);
      formData.append('toId', toId);
      formData.append('type', type);
      formData.append('width', imageSize.width.toString());
      formData.append('height', imageSize.height.toString());
      let data = sendImageMessage(formData);
    };
  }
}
</script>

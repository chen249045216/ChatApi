...
export default class GenalActive extends Vue {
  ...
  @chatModule.State('signal') signal: signalR.HubConnection;
  ...
  exitGroup() {
    this.signal.invoke('ExitGroup', this.activeRoom.groupId);
  }

  exitFriend() {
    this.signal.invoke('ExitFriend', this.activeRoom.userId);
  }
}
</script>
